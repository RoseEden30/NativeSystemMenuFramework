...
namespace VanillaSettings
{
    namespace
    {
        ...
        void Commit(Setting& a_setting, const char* a_why)
        {
            ...
            --g_pendingCommits;
            logger::debug("VanillaSettings: commit '{}' [{}] = {} ({})", a_setting.label,
                a_setting.owner.empty() ? "unnamed" : a_setting.owner.c_str(), a_setting.commitValue, a_why);
        ...
        void QueueCommit(Setting& a_setting, float a_value)
        {
            ...
            if (a_setting.type != Type::kSlider) {
                logger::debug("VanillaSettings: commit '{}' [{}] = {} (no drag)", a_setting.label,
                    a_setting.owner.empty() ? "unnamed" : a_setting.owner.c_str(), a_value);
        ...
        // Registration normally happens at load, but nothing stops a mod
        // calling Add* later or off the menu thread. Recursive because the
        // tick calls into mod code while holding it, and that code may
        // register something in turn.
        std::recursive_mutex g_settingsMutex;

        // Appended after the native tabs, which vanilla's itemPress handler
        // addresses by index.
        std::vector<std::string> g_customTabs;

        // The tabs vanilla ships, in the order its own switch(selectedIndex)
        // expects them. Custom tabs are appended after these, so every index
        // below is measured against this count rather than written down.
        constexpr const char* kNativeTabs[] = { "Gameplay", "Display", "Audio" };
        constexpr std::size_t kNativeTabCount = std::size(kNativeTabs);

        // The index vanilla's own handler expects, or -1 for a custom tab.
        ...
        // A tab belongs to whoever created it. Two mods can share a tab, but
        // only one can own where it sits.
        std::unordered_map<std::string, std::string> g_tabOwners;

        // Shown while the tab is highlighted in the category list.
        std::unordered_map<std::string, std::string> g_tabDescriptions;

        // Which tabs a System menu entry is showing on its own. The full
        // list to put back is parked on the clip itself: a GFxValue held in a
        // global outlives the movie, and destroying it afterwards crashes.
        std::vector<std::string> g_scopedTabs;
        constexpr const char*    kFullTabsMember = "__nsmf_fullTabs";

        // A tab whose mod never named itself is its own group, so unnamed
        // tabs land predictably instead of clumping together.
        const std::string& OwnerKey(const std::string& a_tab)
        {
            const auto it = g_tabOwners.find(a_tab);
            return it != g_tabOwners.end() && !it->second.empty() ? it->second : a_tab;
        }

        bool TabPrecedes(const std::string& a_lhs, const std::string& a_rhs)
        {
            return Ordering::OwnerPrecedes(OwnerKey(a_lhs), OwnerKey(a_rhs));
        }

        std::size_t CustomTabIndex(const std::string& a_tab, const std::string& a_owner = {})
        {
            const auto it = std::find(g_customTabs.begin(), g_customTabs.end(), a_tab);
            if (it != g_customTabs.end())
                return static_cast<std::size_t>(it - g_customTabs.begin());
            g_customTabs.push_back(a_tab);
            g_tabOwners[a_tab] = a_owner;
            return g_customTabs.size() - 1;
        }
        ...
        // Tick() re-injecting its rows over a custom tab.
        bool                          g_showingCustomTab = false;

        // onCancelPress always returns OPTIONS_LISTS_STATE to the tab
        // selector, even when JumpToTab skipped past it. Tick() watches for
        // that and continues the cancel so one press reaches MAIN_STATE.
        bool g_bounceCancelToMain = false;
            ...
            if (a_params.GetArgCount() >= 2 && a_params[0].IsNumber()) {
                ...
                if (id >= kIdBase) {
                    ...
                    if (idx < g_settings.size()) {
                        const auto& setting = g_settings[idx];
                        logger::debug("VanillaSettings: OptionChange '{}' in '{}' [{}] = {} (id={})",
                            setting.type == Type::kLabel ? "(label)" : setting.label.c_str(), setting.tab,
                            setting.owner.empty() ? "unnamed" : setting.owner.c_str(), value, id);
                    } else {
                        logger::warn("VanillaSettings: OptionChange for unknown id {} (idx {})", id, idx);
                    }
                ...
                if (Config::IsVerboseLoggingEnabled()) {
                    std::string args;
                    for (std::uint32_t i = 0; i < a_params.GetArgCount(); ++i) {
                        if (!args.empty())
                            args += ", ";
                        args += a_params[i].IsNumber()      ? std::to_string(a_params[i].GetNumber())
                                : a_params[i].IsString()     ? a_params[i].GetString()
                                : a_params[i].IsBool()       ? (a_params[i].GetBool() ? "true" : "false")
                                                             : "?";
                    }
                    logger::debug("VanillaSettings: vanilla OptionChange id {} raw args [{}]", id, args);
                }
            }
        ...
        void InstallHooks(RE::FxDelegate* a_fxDelegate)
        {
            ...
            g_hooked = true;
            logger::debug("VanillaSettings: OptionChange/Request*Options hooked");
        }


        // Room on screen, measured from the stage: the list's distance to
        // the bottom minus the margin it has at the top, which is how the
        // page centres it. The panel's own _height measures content, not
        // space, so it grows with every entry. -1 when it can't be worked
        // out.
        ...
        {
            const auto& label = Translations::Resolve(a_setting.label);
        ...
        constexpr const char* kFullBorderMember = "__nsmf_fullBorderHeight";
        ...
        RE::GFxValue BuildEntry(RE::GFxMovie* a_view, const Setting& a_setting, std::size_t a_settingIndex)
        {
            ...
            const auto movieType =
                a_setting.type == Type::kButton ? static_cast<double>(Type::kCheckbox) : static_cast<double>(a_setting.type);
        ...
        // Vanilla shows no description for a setting, so the framework adds
        // one: a text field under the rows, following whatever is selected.
        // Built once per menu open; its position is refreshed every tick
        // because each tab shows a different number of rows.
        // Category entries carry a translated label, not the tab name, so
        // position is what identifies them. Empty when the index has no tab.
        std::string TabAt(int a_index)
        {
            ...
            if (!g_scopedTabs.empty())
                return index < g_scopedTabs.size() ? g_scopedTabs[index] : std::string{};
            if (index < kNativeTabCount)
                return kNativeTabs[index];

            const auto custom = index - kNativeTabCount;
            return custom < g_customTabs.size() ? g_customTabs[custom] : std::string{};
        }
        ...
        // border._height is what caps how many rows render - see InjectSettingsList.
        // Taking the description's band out of it drops the rows that would sit
        // there, so the field is the same size on any interface.
        void ReserveDescriptionBand(RE::GFxValue& a_list, double a_rowHeight)
        {
            RE::GFxValue border;
            if (a_rowHeight <= 0.0 || !a_list.GetMember("border", &border) || !border.IsObject())
                return;

            RE::GFxValue full;
            if (!border.GetMember(kFullBorderMember, &full) || !full.IsNumber()) {
                RE::GFxValue current;
                if (!border.GetMember("_height", &current) || !current.IsNumber())
                    return;
                full = current;
                border.SetMember(kFullBorderMember, full);
            }

            const auto target = full.GetNumber() - a_rowHeight * Config::GetDescriptionRows();
            if (target <= a_rowHeight)
                return;

            RE::GFxValue current;
            if (!border.GetMember("_height", &current) || !current.IsNumber() ||
                std::abs(current.GetNumber() - target) <= 0.5)
                return;

            // Nothing re-counts the rendered rows on its own.
            border.SetMember("_height", RE::GFxValue(target));
        ...
        void RefreshDescription(RE::GFxValue& a_list, bool a_isTabList = false)
        {
            ...
            // Ours by ID; vanilla's by its raw "$..." key, which survives
            // translation - the row IDs don't, they are FormIDs on the Audio
            // tab.
            std::string description;
            const auto  index = selectedIdx.GetNumber();
            if (index >= 0.0 && index < static_cast<double>(entries.GetArraySize())) {
                if (a_isTabList) {
                    const auto tab = g_tabDescriptions.find(TabAt(static_cast<int>(index)));
                    if (tab != g_tabDescriptions.end())
                        description = tab->second;
                }

                RE::GFxValue entry, idVal, textVal;
                if (!a_isTabList && entries.GetElement(static_cast<std::uint32_t>(index), &entry) &&
                    entry.IsObject()) {
                    const bool ours = entry.GetMember("ID", &idVal) && idVal.IsNumber() &&
                    ...
                    } else if (entry.GetMember("text", &textVal) && textVal.IsString()) {
                        if (const auto* known = VanillaDescriptions::Find(textVal.GetString()))
                            description = known;
                    }
                }
            }
            ...
            // interface uses - half a row reached past the border, a quarter
            // brushed the chevron.
            //
            ...
            ReserveDescriptionBand(a_list, rowH.GetNumber());
            ...
            RE::GFxValue field;
            if (!a_list.GetMember("__nsmf_description", &field) || !field.IsObject()) {
                // Nothing to build until there is something to say.
                if (description.empty())
                    return;

                // Two rows tall, so a sentence can wrap.
                const RE::GFxValue args[6] = { RE::GFxValue("__nsmf_description"), RE::GFxValue(23000.0),
                    RE::GFxValue(x), RE::GFxValue(y), RE::GFxValue(width),
                    RE::GFxValue(height) };
                a_list.Invoke("createTextField", nullptr, args, 6);

                if (!a_list.GetMember("__nsmf_description", &field) || !field.IsObject()) {
                    logger::warn("VanillaSettings: couldn't create the description field");
                    return;
                }
            ...
            if (Config::IsVerboseLoggingEnabled()) {
                static std::string lastTab, lastOptions;
                auto&              last = a_isTabList ? lastTab : lastOptions;

                auto line = std::format("at ({},{}) size {}x{} | row h={} shown={}", x, y, width, height,
                    rowH.GetNumber(), shownV.GetNumber());
                if (line != last) {
                    last = std::move(line);
                    logger::debug("VanillaSettings: description {}", last);
                }
            }
        }
        ...
        // The mod owning the setting is the source of truth, so its value is
        // pulled back whenever the clip disagrees - a widget replaying frames
        // can come back stale, and a click on a blocked row still moves it
        // before OptionChange is dispatched. Player edits reach the backend
            ...
            case Type::kSlider:
                ...
                if (a_setting.formatValue) {
                    ...
                    if (a_clip.GetMember("value", &value) && value.IsNumber())
                        a_clip.SetMember("text",
                            Text::MakeGFxString(FormatLabel(a_setting, static_cast<float>(value.GetNumber()))));
                }
                break;
        ...
        void CommitSettled(RE::GFxValue& a_list, bool a_haveList)
        {
            ...
            const auto now = std::chrono::steady_clock::now();
            // By index: Commit calls mod code that may register a row, and a
            // deque keeps its references across a push_back but not iterators.
            for (std::size_t i = 0; i < g_settings.size() && g_pendingCommits > 0; ++i) {
        ...
        void InjectNativeTab(RE::GFxMovie* a_view, RE::GFxValue& a_list, const std::string& a_tab)
        {
            ...
            // scrolling stops short of the ones we appended.
            a_list.Invoke("CalculateMaxScrollPosition");
            ...
            // repairing the row ourselves.
            a_list.Invoke("UpdateList");
            RefreshRowAppearance(a_list);
            logger::info("VanillaSettings: injected {} setting(s) into '{}'", added, a_tab);
        }

        // Nothing native populates OptionsList for a custom tab, so build
        // its entryList the same way SystemPage does for its own.
        void ShowCustomTab(RE::GFxMovie* a_view, RE::GFxValue& a_page, const std::string& a_tab)
        {
            RE::GFxValue list;
            if (!GetOptionsList(a_page, list)) {
                logger::debug("VanillaSettings: ShowCustomTab('{}') - OptionsList not found", a_tab);
                return;
            }
            ...
            // EntriesA, not entryList (that's CategoryList's field), and
            ...
            logger::info("VanillaSettings: showing custom tab '{}' ({} setting(s))", a_tab, count);
        }
        ...
        public:
            ...
            {
                if (!a_params.thisPtr) {
                    logger::debug("VanillaSettings: SettingsPressHandler - no thisPtr");
                    return;
                }
                RE::GFxValue page;
                if (!a_params.thisPtr->GetMember("__nsmf_page", &page)) {
                    logger::debug("VanillaSettings: SettingsPressHandler - __nsmf_page not found");
                    return;
                }
                ...
                if (!page.GetMember("SettingsPanel", &panel) || !panel.IsObject() ||
                    ...
                    logger::debug("VanillaSettings: SettingsPressHandler - couldn't read selectedIndex");
                    return;
                }
                ...
                logger::debug("VanillaSettings: tab press [{}] '{}'", selected, tab.empty() ? "?" : tab);
                if (tab.empty())
                    return;

                // A narrowed list holds nothing but that mod's tabs, so even a
                // native name there is one of its own.
                if (g_scopedTabs.empty() && IsNativeTab(tab)) {
                    RE::GFxValue options;
                    if (GetOptionsList(page, options))
                        ScrollToTop(options);
        ...
        bool PressedOnCheckbox(RE::GFxValue& a_clip, RE::GFxValue& a_checkBox)
        {
            ...
            if (!a_clip.GetMember("_xmouse", &mouseX) || !mouseX.IsNumber() ||
                ...
                logger::debug("VanillaSettings: PressedOnCheckbox - couldn't measure the box");
                return true;
            }
        ...
        void ToggleVanillaCheckbox(RE::GFxValue& a_list)
        {
            ...
            if (!a_list.GetMember("EntriesA", &entries) || !entries.IsArray() ||
                ...
                logger::debug("VanillaSettings: ToggleVanillaCheckbox - not a checkbox row or read failed");
                return;
            }
            ...
            if (!ClipForEntry(a_list, entry, clip) || !clip.GetMember("CheckBox_mc", &checkBox) ||
                !checkBox.IsObject()) {
                logger::debug("VanillaSettings: ToggleVanillaCheckbox - no clip for the selected row");
                return;
            }
            ...
            RE::GFxValue idVal, valueVal;
            if (clip.GetMember("ID", &idVal) && idVal.IsNumber() && clip.GetMember("value", &valueVal) &&
                valueVal.IsNumber()) {
                logger::debug("VanillaSettings: ToggleVanillaCheckbox - id {} value {} -> {}", idVal.GetNumber(),
                    valueVal.GetNumber(), valueVal.GetNumber() != 0.0 ? 0.0 : 1.0);
            }
        ...
        {
            if (g_settingsListInjected || g_customTabs.empty())
                return;
            ...
            if (!haveList) {
                logger::debug("VanillaSettings: SettingsPanel found={} List_mc found={}", havePanel, haveList);
                return;
            }
            ...
            if (!haveEntryList || entryCount < kNativeTabCount) {
                logger::debug("VanillaSettings: SettingsList.entryList found={} size={} - not populated yet",
                    haveEntryList, entryCount);
                return;
            }

            std::stable_sort(g_customTabs.begin(), g_customTabs.end(), TabPrecedes);

            for (const auto& tab : g_customTabs) {
                RE::GFxValue entry;
                a_view->CreateObject(&entry);
                entry.SetMember("text", Text::MakeGFxString(tab));
                entryList.Invoke("push", nullptr, &entry, 1);
            }

            Debug::LogMembers(panel, "SettingsPanel");
            Debug::LogHelpPanel(a_page);
            Debug::LogMembers(list, "SettingsList");
            ...
            // same height, reports nothing to scroll - leaving those tabs both
            ...
            if (list.GetMember("Entry0", &entry0) && list.GetMember("border", &border) && border.IsObject()) {
                ...
                if (entry0.GetMember("_height", &rowHeight) && rowHeight.IsNumber() && rowHeight.GetNumber() > 0.0 &&
                    ...
                    logger::debug(
                        "VanillaSettings: SettingsList row {} tall, border {}, panel {} - growing border to {}",
                        rowHeight.GetNumber(), curHeight.GetNumber(), panelHeight.GetNumber(), grown);
            ...
            list.Invoke("InvalidateData");
            // Without this the appended rows exist in the data but stay
            ...
            scope.SetMember("onNSMFSettingsPress", fn);
            ...
            const RE::GFxValue add[3] = { RE::GFxValue("itemPress"), scope, RE::GFxValue("onNSMFSettingsPress") };
            list.Invoke("addEventListener", nullptr, add, 3);

            g_settingsListInjected = true;
            list.SetMember(kFullTabsMember, entryList);
            for (const auto& tab : g_customTabs) {
                const auto owner = g_tabOwners.find(tab);
                logger::debug("VanillaSettings: tab '{}' owned by '{}'", tab,
                    owner != g_tabOwners.end() && !owner->second.empty() ? owner->second : "(unnamed)");
            }
            logger::info("VanillaSettings: added {} custom tab(s) to the Settings list", g_customTabs.size());
        }
    }
    ...
    void Tick(RE::JournalMenu* a_this, RE::GFxMovieView* a_view, RE::GFxValue& a_systemPage)
    {
        ...
        if (g_bounceCancelToMain) {
            const auto settingsCategory = SystemState::Read(a_view, "SETTINGS_CATEGORY_STATE", 3.0);
            const auto optionsLists = SystemState::Read(a_view, "OPTIONS_LISTS_STATE", 4.0);
            const auto transitioning = SystemState::Read(a_view, "TRANSITIONING", 15.0);

            RE::GFxValue stateVal;
            if (a_systemPage.GetMember("currentState", &stateVal) && stateVal.IsNumber()) {
                const auto state = stateVal.GetNumber();
                if (state == settingsCategory) {
                    // onSettingsCategoryPress marks the settings dirty just by
                    // opening a category, and that flag sends the cancel down
                    // vanilla's one-second "$Saving..." path. A custom tab
                    // holds nothing vanilla saves; native tabs keep the flag.
                    if (g_showingCustomTab)
                        a_systemPage.SetMember("bSettingsChanged", RE::GFxValue(false));
                    a_systemPage.Invoke("onCancelPress");
                    g_bounceCancelToMain = false;
                } else if (state != optionsLists && state != transitioning) {
                    g_bounceCancelToMain = false;
                }
            }
        }
        ...
        if (!g_showingCustomTab && g_haveCurrentTab && IsNativeTab(g_currentTab) && haveList) {
            Debug::LogVanillaEntries(list, g_currentTab);
        ...
        if (haveList && !g_optionsPressHooked) {
            ...
            scope.SetMember("onNSMFOptionPress", fn);
            scope.SetMember("__nsmf_page", a_systemPage);

            // Added, not swapped: vanilla's own handler still runs.
            const RE::GFxValue add[3] = { RE::GFxValue("itemPress"), scope, RE::GFxValue("onNSMFOptionPress") };
            list.Invoke("addEventListener", nullptr, add, 3);
            logger::debug("VanillaSettings: OptionsList itemPress hooked");
        }
        ...
        if (haveList) {
            ...
            static bool loggedOptionsList = false;
            if (!loggedOptionsList) {
    ...
    bool AddSetting(std::string a_tab, Type a_type, std::string a_label, std::function<float()> a_getValue,
        ...
        std::string a_description, std::string a_owner, std::function<void(float)> a_onCommit)
    {
        const std::lock_guard lock(g_settingsMutex);
        ...
        if (a_type != Type::kSlider && a_type != Type::kDropdown && a_type != Type::kCheckbox) {
            logger::warn("VanillaSettings: '{}' asked for type {} - use AddLabel or AddButton", a_label,
                static_cast<int>(a_type));
            return false;
        }
        Translations::Load(a_owner);
        if (!IsNativeTab(a_tab))
            CustomTabIndex(a_tab, a_owner);
        g_settings.push_back({ std::move(a_tab), a_type, std::move(a_label), std::move(a_getValue),
            ...
            Align::kLeft, std::move(a_owner) });
        return true;
    }

    bool AddLabel(std::string a_tab, std::function<void(char*, int)> a_getText, Align a_align, std::string a_owner)
    {
        const std::lock_guard lock(g_settingsMutex);
        ...
        g_settings.push_back({ std::move(a_tab), Type::kLabel, {}, nullptr, nullptr, nullptr, 0.0f, {}, nullptr,
            nullptr, std::move(a_getText), nullptr, {}, a_align, std::move(a_owner) });
        return true;
    }

    bool AddButton(std::string a_tab, std::string a_label, std::function<void()> a_onPress, std::string a_owner)
    {
        const std::lock_guard lock(g_settingsMutex);
    ...
    bool SetTabDescription(std::string a_tab, std::string a_description)
    {
        ...
        if (a_description.empty())
            g_tabDescriptions.erase(a_tab);
        else
            g_tabDescriptions[std::move(a_tab)] = std::move(a_description);
        return true;
    }
    ...
    bool ShowOwnerTabs(RE::GFxMovieView* a_view, RE::GFxValue& a_page, const std::string& a_owner)
    {
        ...
        if (a_owner.empty() || !a_view)
            return false;

        RE::GFxValue panel, list;
        if (!a_page.GetMember("SettingsPanel", &panel) || !panel.IsObject() ||
            !panel.GetMember("List_mc", &list) || !list.IsObject())
            return false;

        // g_customTabs is already grouped, so one owner's tabs are contiguous
        // and come out in the order that mod declared them.
        std::vector<std::string> tabs;
        for (const auto& tab : g_customTabs) {
            if (OwnerKey(tab) == a_owner)
                tabs.push_back(tab);
        }
        if (tabs.empty()) {
            logger::warn("VanillaSettings: '{}' has no tabs to show", a_owner);
            return false;
        }
        ...
        // entryList is a plain alias for EntriesA, so this swaps the whole
        // list in one write. The full one stays on the clip, under
        // kFullTabsMember.
        list.SetMember("entryList", entryList);
        g_scopedTabs = std::move(tabs);
        ...
        a_page.Invoke("StartState", nullptr, &state, 1);

        logger::info("VanillaSettings: showing '{}' own tab list ({} tab(s))", a_owner, g_scopedTabs.size());
        return true;
    }
    ...
    void ClearOwnerScope(RE::GFxValue& a_page)
    {
        ...
        RE::GFxValue panel, list, full;
        if (!a_page.GetMember("SettingsPanel", &panel) || !panel.IsObject() ||
            !panel.GetMember("List_mc", &list) || !list.IsObject() ||
            !list.GetMember(kFullTabsMember, &full) || !full.IsArray())
            return;

        list.SetMember("entryList", full);
        list.Invoke("InvalidateData");
        logger::debug("VanillaSettings: full tab list restored");
    }
    ...
    void JumpToTab(RE::GFxMovie* a_view, RE::GFxValue& a_page, const std::string& a_tab)
    {
        ...
        if (IsNativeTab(a_tab)) {
            RE::GFxValue panel, list;
            if (!a_page.GetMember("SettingsPanel", &panel) || !panel.IsObject() || !panel.GetMember("List_mc", &list) ||
                !list.IsObject())
                return;
            list.SetMember("selectedIndex", RE::GFxValue(static_cast<double>(NativeTabIndex(a_tab))));
            a_page.Invoke("onSettingsCategoryPress");
            g_currentTab = a_tab;
            g_haveCurrentTab = true;
            g_showingCustomTab = false;
        } else {
            ShowCustomTab(a_view, a_page, a_tab);
        }
        g_bounceCancelToMain = true;
    }
}
